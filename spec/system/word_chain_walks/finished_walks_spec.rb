...
RSpec.describe 'FinishedWalks', type: :system do
  ...
  let!(:word_chain_walk_step) { FactoryBot.create(:word_chain_walk_step, :with_image, word_chain_walk: word_chain_walk, word: 'りんご', latitude: 35.6586, longitude: 139.7454) }
  ...
  scenario '地図のマーカーをクリックすると対応する記録の情報が表示される' do
    ...
    within '#walk-map-container' do
      find("a[href='#{word_chain_walk_word_chain_walk_step_path(word_chain_walk, word_chain_walk_step)}']").click
    end
  ...
  scenario '地図のピンを押して表示されたカードから記録の詳細画面に移動できる' do
    ...
    word_chain_walk_step_path = word_chain_walk_word_chain_walk_step_path(word_chain_walk, word_chain_walk_step)

    within '#walk-map-container' do
      find("a[href='#{word_chain_walk_step_path}']").click
    end
    ...
    expect(page).to have_current_path(word_chain_walk_step_path)
    expect(page).to have_content('りんご')
  end
end
